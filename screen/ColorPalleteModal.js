...
const ColorPalleteModel = ({navigation}) => {
...
const handleSubmit=useCallback(()=>{
  if(!name){
    Alert.alert('Please Enter Pallete Name');
  }
  else if(selectcolor.length<3){
    Alert.alert('Please Enter Atleast 3 Color');
    ...
    navigation.navigate('Home',{ addcolorpallete })
  }
},[name,selectcolor]);

const handlevaluechange=useCallback((value,color)=>{
  if(value===true){
    setselectcolor(colors=>[...colors,color])
  }
  else{
    setselectcolor(colors=>colors.filter(selectcolor=>color.colorName!==selectcolor.colorName))
  }
},[])
  return (
    <View style={styles.Container}>
      <Text style={styles.namet}>Enter PalleteName</Text>
        <TextInput style={styles.input} value={name} onChangeText={setname} placeholder="Pallete Name"/>
        <FlatList
            data={COLORS}
          ...
          <View style={styles.color}>
            ...
            <Switch 
              value={
                !!selectcolor.find(color=>color.colorName===item.colorName)}
...
export default ColorPalleteModel;